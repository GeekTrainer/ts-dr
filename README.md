...
The game catalog can be filtered immediately by one or more categories and by a publisher. Filter selections are stored in the URL query string, so filtered catalog views can be bookmarked, shared, and restored after a reload.
